 ...
 */
@Plugin(name = ScriptrunningDailyRollingFileAppender.PLUGIN_NAME, category = Core.CATEGORY_NAME, elementType = Appender.ELEMENT_TYPE, printObject = true)
public class ScriptrunningDailyRollingFileAppender extends NonFinalRollingFileAppender {
...
//		super.activateOptions();
		...
		if (script == null) {
		    log.error("Script option is not set for appender ["+name+"].");				
		}
	}
	...
	public void run() {
		...
		try {
            ...
            try {
                int e = p.exitValue(); 
                if (e != 0) {
                    log.error("Script did not terminate with 0 return value.");                    
                    System.out.println("Script did not terminate with 0 return value.");                    
                }
            } catch (IllegalThreadStateException e) {
                log.error("Script did not terminate within the timeout, 60 seconds.");
                System.out.println("Script did not terminate within the timeout, 60 seconds.");
                p.destroy();
            }
            */
		} catch (Exception e) {
		    ScriptrunningDailyRollingFileAppender.log.error("Exception caught while running script: ", e);
			e.printStackTrace();
		} 
